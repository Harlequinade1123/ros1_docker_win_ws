...
2. 初回起動時のウィザードで **「Use WSL 2 instead of Hyper-V」** を選択

### リソース制限の設定（推奨）

Docker Desktop の Settings → Resources で以下を設定します。

| 項目 | 推奨値 |
|---|---|
| CPU | ホスト CPU の半分以下 |
| Memory | 4 GB 以上（Gazebo 使用時は 8 GB 推奨） |
| Swap | 2 GB |
| Disk image size | 50 GB 以上 |
