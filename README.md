...
| OS | 操作ファイル | セットアップ手順 |
|---|---|---|
| Linux / macOS | `Makefile`（`make` コマンド） | このページを参照 |
| **Windows** | `run.ps1`（PowerShell スクリプト） | **[docs/windows-setup.md](docs/windows-setup.md)** |

> Windows をお使いの場合は **[Windows セットアップガイド](docs/windows-setup.md)** を最初にご覧ください。  
> Git・Docker Desktop・VcXsrv のインストールから初回ビルドまで手順をまとめています。
...
> Windows の詳細なセットアップ（Git/Docker/VcXsrv のインストール等）は [docs/windows-setup.md](docs/windows-setup.md) を参照してください。
...
> **Windows（WSL2）では非対応**: SocketCAN は Linux カーネルの機能です。WSL2 のデフォルトカーネルには CAN モジュールが含まれておらず、Windows 環境では使用できません。CAN 通信には **Linux 環境**を使用してください。
