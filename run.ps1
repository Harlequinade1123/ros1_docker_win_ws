...
switch ($Command.ToLower()) {
    ...
    "can-up" {
        Write-Error "CAN 通信は Windows（WSL2）では非対応です。Linux 環境で make can-up を使用してください。"
        exit 1
    }

    "can-down" {
        Write-Error "CAN 通信は Windows（WSL2）では非対応です。Linux 環境で make can-down を使用してください。"
        exit 1
    }

    "can-status" {
        Write-Error "CAN 通信は Windows（WSL2）では非対応です。Linux 環境で make can-status を使用してください。"
        exit 1
    }

    "can-dump" {
        Write-Error "CAN 通信は Windows（WSL2）では非対応です。Linux 環境で make can-dump を使用してください。"
        exit 1
    }

    "can-send" {
        Write-Error "CAN 通信は Windows（WSL2）では非対応です。Linux 環境で make can-send を使用してください。"
        exit 1
    }
...
[CAN通信]  ※ Windows（WSL2）では非対応 — Linux 環境（make コマンド）を使用してください
